...
输入：packA(预打包A)[M_packed, K], B[N,K] (fp16)；约束设置为M%16==0, N%8==0。
...
2. K按vl（向量长度）分块：
   - 加载A[j+0..3][k..k+vl]到j0..j3。
   - 加载B[i+0..3][k..k+vl]到i0..i3。
...
│           └─ 每个Q8_0 block：K0..7/K8..15/K16..23/K24..31 → SMMLA
│              int32→fp32 → ×A scale → ×B scale → 累加到C
```
...
开源llama.cpp Attention由 QKᵀ、缩放、Mask、Softmax、PV等多个独立算子依次完成。中间 scores矩阵被反复读写，CPU上易从“算力瓶颈”变为“L2/L3/内存带宽瓶颈”。Fused SDPA将这些步骤融合进一个专用C++/NEON/SVE kernel，按tile流式完成，减少中间张量与内存流量。
...
满足条件时构建`GGML_OP_FUSED_CPP_SDPA_EXT`op替代开源flash_attn。
...
│  └─ NO →开源flash_attn（flash_attn_ext）
```
...
优化补丁还包含对基线源码的若干功能性修复，用于保证精度验证的公平性。
...
| 环境变量 | 值 | 效果 |
| --- | --- | --- |
| `GGML_FUSED_CPP_SDPA` | `0`/`off`/`false`/`no` | 关闭fused SDPA，回退到开源flash_attn |
| `GGML_FUSED_CPP_SDPA` | `debug`/`trace` | 启用fused SDPA调试日志 |
| `GGML_FUSED_CPP_SDPA` | 空/其他 | 默认启用 |

## 验收标准

- 功能正确性：优化后各kernel输出与开源模型输出在指定测试数据集上余弦相似度大于0.999或 C-MTEB所有数据集平均得分掉点小于1%，且通过精度验证脚本验证。
